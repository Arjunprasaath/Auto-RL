...
# (1) Early stopping — exit after this many consecutive chunks with no improvement
STAGNATION_LIMIT = 8
# (4/5) Race dropout — (budget_fraction, peer_fraction) checkpoints
# At 33% of budget: exit if reward < 20% of best peer's reward
# At 66% of budget: exit if reward < 40% of best peer's reward
DROPOUT_SCHEDULE = [(0.33, 0.20), (0.66, 0.40)]
# Minimum chunks before dropout logic activates (let training warm up first)
...
def _race_dropout_check(
    ...
    """Return True if this agent should exit early due to race dropout."""
    for threshold, peer_fraction in DROPOUT_SCHEDULE:
        ...
        if progress >= threshold:
            ...
            try:
                ...
                # Only dropout if someone else is clearly winning and has positive reward
...
def main():
    ...
    # (3) W&B Artifact warm-start: create model with desired hparams, then
    # overwrite policy weights from the best previous run if one exists.
    model = PPO(
        policy, env,
    ...
    if warm_ckpt:
        try:
            warm_model = PPO.load(warm_ckpt, env=env, device=a.device)
    ...
    # Lazy-import coordinator for race dropout
    try:
    ...
    try:
        while time.time() - start < a.time_budget:
            ...
            # (1) Early stopping: exit if reward hasn't improved for STAGNATION_LIMIT chunks
            ...
            # (4/5) Hyperband / race dropout via Redis
    ...
    # Evaluate on whatever we have trained so far (even if we exited early)
