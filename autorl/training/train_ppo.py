"""
PPO training script — runs locally on MuJoCo (Person A).
...
Run from the autorl/ package root, e.g.:
    python training/train_ppo.py --agent-id test_ppo --env-id HalfCheetah-v5 \
...
@weave.op(name="PPO_Training")
def train_ppo(agent_id, env_id, time_budget, lr, seed, results_dir):
    """Time-budgeted PPO training. Traced as a Weave op when tracing is on."""
    os.makedirs(f"{results_dir}/{agent_id}", exist_ok=True)

    hb = HeartbeatWriter(agent_id, results_dir)
    hb.start()

    model = PPO("MlpPolicy", env_id, learning_rate=lr, seed=seed, verbose=0)
